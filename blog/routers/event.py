...
@router.post('/')
def create(request: schema.Event, db: Session = Depends(get_db)):
...
def delete_event(id,db:Session=Depends(database.get_db)):
   return event.delete_event(id,db)


@router.put('/{id}',status_code=status.HTTP_202_ACCEPTED,)
def update(id,request:schema.Event,db:Session=Depends(get_db)):
